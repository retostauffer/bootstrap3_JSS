				...
				{if $canRecordDecision}
					...
					<script>
					 $(document).ready(function() {
						$("#decision_shower").on("click", function(){
							document.querySelector("#workflow_decision_options").classList.remove("pkp_workflow_decisions_options_hidden")
							document.querySelector("#decision_shower").classList.add("pkp_workflow_decisions_options_hidden")
						})
					}
					)
					</script>

					{*span added by this theme (button to hide decision options)*}
					<span id="decision_shower" class="pkp_button_offset{if $lastDecision} pkp_workflow_decisions_options_hidden{/if}">Click here to make a decision now!</span>
					{*ul modified by this theme (class pkp_workflow_decisions_options_hidden added by default, logic moved to span hiding this ul)*}
					<ul id="workflow_decision_options" class="pkp_workflow_decisions_options pkp_workflow_decisions_options_hidden">
